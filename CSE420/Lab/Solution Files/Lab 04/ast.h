...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        outcode << "if " << cond_temp << " goto L" << if_label << endl;
        if (else_block) {
        ...
        outcode << "L" << if_label << ":" << endl;
        string then_temp = then_block->generate_code(outcode, symbol_to_temp, temp_count, label_count);
        
        if (else_block) {
            outcode << "goto L" << end_label << endl;
            outcode << "L" << else_label << ":" << endl;
            string else_temp = else_block->generate_code(outcode, symbol_to_temp, temp_count, label_count);
        }
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        outcode << "goto L" << loop_label << endl;
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        for (const auto& var : vars) {
            ...
            if (var.second > 0) {
                outcode << var.first << "[" << var.second << "]" << endl;
            } else {
                outcode << var.first << endl;
            }
        }
